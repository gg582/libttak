...
#define V_HEADER(ptr) do { \
    ...
    if (_h->magic != TTAK_MAGIC_NUMBER || _h->checksum != ttak_calc_header_checksum(_h)) { \
        fprintf(stderr, "[FATAL] TTAK Memory Corruption detected at %p\n", (void*)ptr); \
        abort(); \
    } \
} while(0)
...
void *ttak_mem_alloc_safe(size_t size, uint64_t lifetime_ticks, uint64_t now, _Bool is_const, _Bool is_volatile, _Bool allow_direct, ttak_mem_flags_t flags) {
    ...
    if (flags & TTAK_MEM_HUGE_PAGES) {
        header = mmap(NULL, total_size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB, -1, 0);
    ...
    if (!header) {
        if (flags & TTAK_MEM_CACHE_ALIGNED) {
            if (posix_memalign((void **)&header, 64, total_size) != 0) {
                header = NULL;
            }
        } else {
    ...
    if (!header && errno == ENOMEM) {
        ...
        if (!retrying) {
            ...
            void *res = ttak_mem_alloc_safe(size, lifetime_ticks, now, is_const, is_volatile, allow_direct, flags);
            retrying = false;
            return res;
        }
    }
    ...
    __atomic_add_fetch(&global_mem_usage, total_size, __ATOMIC_SEQ_CST);
...
void *ttak_mem_realloc_safe(void *ptr, size_t new_size, uint64_t lifetime_ticks, uint64_t now, ttak_mem_flags_t flags) {
    V_HEADER(ptr);
    if (!ptr) {
        return ttak_mem_alloc_safe(new_size, lifetime_ticks, now, false, false, true, flags);
    }
    ...
    void *new_ptr = ttak_mem_alloc_safe(new_size, lifetime_ticks, now, is_const, is_volatile, allow_direct, flags);
